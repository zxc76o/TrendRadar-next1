...
class DataFetcher:
    ...
    def fetch_x_trends(self, alias: str) -> Tuple[Optional[str], str, str]:
        ...
        if BeautifulSoup is None:
            print("获取 X 热点失败: 未安装 beautifulsoup4 依赖")
        ...
        try:
            ...
            if not card:
                raise ValueError("未找到趋势数据")

            items = []
            seen_titles = set()
            limit = CONFIG.get("X_TRENDS_MAX_TOPICS", 20)

            for li in card.select(".trend-card__list li"):
                anchor = li.find("a")
                if not anchor:
                    continue
                ...
                link = self._normalize_x_link(link)

                tweet_count_el = li.select_one(".tweet-count")
                tweet_count = (
                    tweet_count_el.get_text(strip=True) if tweet_count_el else ""
                )
